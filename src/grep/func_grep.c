...
void parcer(int argc, char* argv[], opt* options) {
    for (int i = 1; i < argc; i++) {
        if (argv[i][0] == '-' && strlen(argv[i]) == 1 && options->check_e != 1 && options->check_f != 1) {
            if (i != argc - 1) {
            ...
            case 'i':
                ...
                if (i != argc - 1 && options->templateCounter == 0) {
                    if (options->check_e != 1 && argv[i + 1][0] != '-' && options->check_f != 1) {
                    strcpy(options->templateSave[options->templateCounter], argv[++i]);
                    options->templateCounter++;
                }
                }
            ...
            case 'f':
                ...
                while (fgets(string_save, 1024, f) != NULL) {
                    strcpy(options->templateSave[options->templateCounter], string_save);
                    options->templateCounter++;
                }
                }
                break;
            ...
            for (int j = 1; j < strlen(argv[i]); j++) {
                ...
                case 'e':
                    ...
                    if (j != strlen(argv[i]) - 1) {
                        ...
                        j = strlen(argv[i]);
                        strcpy(options->templateSave[options->templateCounter], result_without_e);
                        options->templateCounter++;
            ...
            if (options->fileCounter == 0 && i == 1 && options->check_e != 1 && options->check_f != 1) {
                strcpy(options->templateSave[options->templateCounter], argv[i]);
                options->templateCounter++; 
            } else {
    ...
    for (int i = 1; i < argc; i++) {
        if (argv[i][0] == '-' && strlen(argv[i]) <= 2) {
            if (argv[i][1] == 'e') {
                ...
                if (options->check_e == 1 && options->stopread != 1 && argc > 3) { // ./s21_grep -e -z -z  // do dont work
                    for (int p = i + 1; p < argc; p++) {
                        if (argv[p][0] != '-') {
                            check_exist_file_after_e_without_stopread = 1; // 1 file exist in line
                        }
                    }
        ...
        if (argv[i][0] == '-' && strlen(argv[i]) > 2) {
            ...
            for (int j = 1; j < strlen(argv[i]); j++) {
                if (argv[i][j] == 'e') {
                    ...
                    if (j == strlen(argv[i]) - 1 && i < argc - 1 && argv[i+1][0] == '-' && exist_controversial_situation_two != 1) {
                        options->stopread = 0;
        ...
        if (argv[i][0] == '-' && strlen(argv[i]) <= 2) {
            if (argv[i][1] == 'f') {
                ...
                if (options->stopread != 1 && i < argc - 1) {
                    ...
                    i++;
                    // DELETE ME printf("\nHERE10\n");
                    // DELETE ME printf("argv[%d] = %s\n", i, argv[i]);

                    if (fileisopen(argv, i) == 0 && i < argc - 1) {
                        // DELETE ME printf("\nHERE11\n");
                        // DELETE ME printf("\n%s - file is open\n", argv[i]);
                        // DELETE ME printf("\nHERE12\n");
                        // check after argv exist file without '-'
...
// HERE WORK WORK WORK HERE WORK WORK WORK HERE WORK WORK WORK HERE WORK WORK WORK HERE WORK WORK WORK
                    } else if (fileisopen(argv, i) == 0 && i == argc - 1 && check_file_before_f == 0) {
                        // DELETE ME printf("\nHERE3\n");
                        options->stopread = 1;
                    } else if (fileisopen(argv, i) == 0 && i == argc - 1 && check_file_before_f != 0) {
                        // DELETE ME printf("\nHERE18\n");
                    } else {
                        options->return_no_such_file_for_f = 1; // DELETE ME
                        options->stopread = 1;
                        // DELETE ME printf("\nHERE17\n");
                    }
                    // DELETE ME printf("\nHERE13\n");
        ...
        if ((file = fopen(options.fileSave[i], "r")) == NULL) {
            ...
            while (fgets(string_save, 1024, file) != NULL) {
                ...
                for (int j = 0; j < options.templateCounter; j++) {
                    ...
                    if (options.i == 1) {
                        int check = regcomp(&regex, options.templateSave[j], REG_ICASE); // prepares the regular expression
                    } else {
                        int check = regcomp(&regex, options.templateSave[j], 0); // prepares the regular expression
                    }
                    int status = regexec(&regex, string_save, 0, NULL, REG_NEWLINE); // 0 - matched
                    //
                    ...
                    if (counter_status_o > 0 && options.v != 1) {
                        if (options.n == 1) {
                            if (options.fileCounter > 1 && options.h != 1) {
                                printf("%s:%d:%s", options.fileSave[i], counter_lines, string_save);
                            } else {
